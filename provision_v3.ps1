...
if (-not (choco list --local-only sysmon | Select-String "sysmon")) {
    ...
    $sysmonConfigUrl = "https://raw.githubusercontent.com/olafhartong/sysmon-modular/master/sysmonconfig-with-filedelete.xml"
    ...
    try {
        ...
        Write-Host "Successfully downloaded Sysmon configuration."
    } catch {
        Write-Host "Failed to download Sysmon configuration: $_"
        throw "Sysmon configuration download failed"
    }

    # Navigate to Sysmon directory and install with config
    ...
    if (Test-Path $sysmonPath) {
        ...
        # Auto-accept EULA and install with config
        Start-Process -FilePath ".\Sysmon64.exe" -ArgumentList "-i", "$sysmonConfigPath", "-accepteula" -Wait -NoNewWindow
        Pop-Location
    } else {
...
# Verify Sysmon and Script Block Logging
Write-Host "Verifying Sysmon installation and configuration..."
Get-Service Sysmon64 | Select-Object Name, Status
Get-ItemProperty -Path 'HKLM:\SOFTWARE\Wow6432Node\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' -Name EnableScriptBlockLogging